      ...
      for (const file of files) {
        try {
          if (!(await VaultCLI.isEncrypted(file))) {
            ...
            this.loadingIndicator.stop(`⚠️ ${file} already encrypted`);
          }
        } catch (error) {
          this.loadingIndicator.stop(`✘ Failed to encrypt ${file}: ${error}`);
        }
      }
    } catch (error) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          this.loadingIndicator.stop(`✘ Failed to decrypt ${file}: ${error}`);
        }
      }
    } catch (error) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          this.loadingIndicator.stop(`✘ Failed to restore ${file}: ${error}`);
        }
      }
    } catch (error) {
  ...
  switch (command) {
    ...
    case VaultCommand.HISTORY:
      if (filenames.length !== 1) {
        throw new Error(`${command} command supports only one file at a time`);
      }
      break;

    case VaultCommand.RESTORE:
      if (filenames.length !== 2) {
        throw new Error('Restore command requires a filename and a version ID');
      }
      break;
    ...
    case VaultCommand.DECRYPT:
      if (filenames.length < 1) {
        throw new Error(`${command} command requires at least one filename`);
      }
      break;
  }
}
  ...
  try {
    ...
    switch (command) {
      ...
      case VaultCommand.RESTORE: {
        ...
        try {
          const password = await VaultCLI.getPassword();
          loadingIndicator.start(`Restoring version ${filenames[1]}...`);
          ...
          const error = err as Error;
          loadingIndicator.stop(`✘ Restoration failed: ${error.message}`);
